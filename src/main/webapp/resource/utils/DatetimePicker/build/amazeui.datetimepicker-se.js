...
var dateTimePicker = _dereq_('../vendor/bootstrap-datetimepicker')($, moment);
...
},{"../vendor/bootstrap-datetimepicker":2}],2:[function(_dereq_,module,exports){
/*! version : 4.17.37
 =========================================================
...
module.exports = function($, moment) {
  ...
  var dateTimePicker = function(element, options) {
    var picker = {},
      ...
      getMoment = function(d) {
        ...
        if (d === undefined || d === null) {
          ...
          if (tzEnabled) {
            ...
            incomingZoneOffset = moment(d, parseFormats, options.useStrict)
              .utcOffset();
            if (incomingZoneOffset !== currentZoneOffset) {
              timeZoneIndicator = moment().tz(options.timeZone).format('Z');
              dateWithTimeZoneInfo = moment(d, parseFormats, options.useStrict)
                  .format('YYYY-MM-DD[T]HH:mm:ss') + timeZoneIndicator;
      ...
      getToolbar = function() {
        ...
        if (!options.sideBySide && hasDate() && hasTime()) {
          ...
          }).append($('<span> ' + options.tooltips.selectTime + '</span>').addClass(options.icons.time))));
        }
        ...
        return $('<table>').addClass('table-condensed')
      ...
      getTemplate = function() {
        var template = $('<div>')
          ...
          dateView = $('<div>').addClass('datepicker')
            .append(getDatePickerTemplate()),
          timeView = $('<div>').addClass('timepicker')
        ...
        if (options.sideBySide && hasDate() && hasTime()) {
          ...
          template.append(
            $('<div class="am-g">')
              .append(dateView.addClass('am-fl'))
              .append(timeView.addClass('am-fl'))
          );
      ...
      place = function() {
        var $refer = component || element;
        var position = $refer.position(),
          offset = $refer.offset(),
        ...
        if (vertical === 'auto') {
          if (offset.top + widget.height() * 1.5 >= $(window)
        ...
        // TODO: 完善定位逻辑
        widget.addClass('bottom ' + horizontal);
        ...
        widget.css($.extend({}, style, {
          left: horizontal === 'left' ? offset.left : offset.left + referWidth - widgetWidth
          // right: horizontal === 'left' ? 'auto' : offset.left
      ...
      fillMonths = function() {
        ...
        while (monthsShort.isSame(viewDate, 'y')) {
          spans.push($('<span>').attr('data-action', 'selectMonth')
            .addClass('month').text(monthsShort.format('MMM')));
          monthsShort.add(1, 'M');
        }
      ...
      updateMonths = function() {
        ...
        monthsViewHeader.eq(0).find('span')
          .attr('title', options.tooltips.prevYear);
        monthsViewHeader.eq(1).attr('title', options.tooltips.selectYear);
        monthsViewHeader.eq(2).find('span')
      ...
      updateYears = function() {
        ...
        yearsViewHeader.eq(0).find('span')
          .attr('title', options.tooltips.prevDecade);
        yearsViewHeader.eq(1).attr('title', options.tooltips.selectDecade);
        yearsViewHeader.eq(2).find('span')
      ...
      updateDecades = function() {
        ...
        decadesViewHeader.eq(0).find('span')
          .attr('title', options.tooltips.prevCentury);
        decadesViewHeader.eq(2).find('span')
      ...
      fillDate = function() {
        ...
        daysViewHeader.eq(0).find('span')
          .attr('title', options.tooltips.prevMonth);
        daysViewHeader.eq(1).attr('title', options.tooltips.selectMonth);
        daysViewHeader.eq(2).find('span')
      ...
      actions = {
        ...
        selectMonth: function(e) {
          var month = $(e.target).closest('tbody').find('span')
            .index($(e.target));
      ...
      show = function() {
        ...
        } else if (options.useCurrent && unset && ((input.is('input') && input.val()
            .trim().length === 0) || options.inline)) {
      ...
      initFormatting = function() {
        ...
        actualFormat = format.replace(/(\[[^\[]*\])|(\\)?(LTS|LT|LL?L?L?|l{1,4})/g, function(formatInput) {
          var newinput = date.localeData()
              .longDateFormat(formatInput) || formatInput;
          return newinput.replace(/(\[[^\[]*\])|(\\)?(LTS|LT|LL?L?L?|l{1,4})/g, function(formatInput2) { //temp fix for #740
            return date.localeData()
                .longDateFormat(formatInput2) || formatInput2;
          });
        });
        ...
        use24Hours = (actualFormat.toLowerCase()
          .indexOf('a') < 1 && actualFormat.replace(/\[.*?\]/g, '')
          .indexOf('h') < 1);
    ...
    picker.defaultDate = function(defaultDate) {
      ...
      if ((options.defaultDate && options.inline) || input.val()
          .trim() === '') {
